...
import {
  Share2, Trash2, Clock, Lock, Eye, Copy, Check, ArrowLeft, Cloud, Files
} from 'lucide-react';

interface BatchShareInfo {
  id: string;
  share_code: string;
  file_ids: string[];
  ...
  created_at: string;
  view_count: number;
  files: { id: string; filename: string; size: number; type: string }[];
}
...
export default function BatchShareManagePage() {
  const { token } = useAuth();
  const [shares, setShares] = useState<BatchShareInfo[]>([]);
  ...
  async function deleteShare(id: string) {
    ...
    try {
      await fetch(`/api/batch-share/manage/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  async function copyLink(shareCode: string, id: string) {
    const link = `${window.location.origin}/batch-share/${shareCode}`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900">
      <header className="sticky top-0 z-40 bg-white/5 backdrop-blur-xl border-b border-white/10 px-4 md:px-6 py-4 flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold text-white">批量分享管理</h1>
      </header>
      ...
      <div className="p-4 md:p-6">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-white/40">
            <Files className="w-16 h-16 mb-4" />
            <p className="text-lg">暂无批量分享</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-white font-medium">{share.files.length} 个文件</span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {share.files.slice(0, 5).map(f => (
                        <span key={f.id} className="text-white/50 text-sm truncate max-w-[150px]">{f.filename}</span>
                      ))}
                      {share.files.length > 5 && (
                        <span className="text-white/40 text-sm">+{share.files.length - 5} 更多</span>
                      )}
                    </div>
                  </div>
